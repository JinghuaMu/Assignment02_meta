...
### Read the study raw data and clean
...
### Generate statistics and merge files
...
### Generate lnRR and sampling variance
...
The values were found to vary greatly between experiments, and  made an attempt to convert them to Log response ratio (lnRR).
...
By screening, we removed 97 of the 824 experiments, which reduced the validity of our meta-analysis but was more conducive to obtaining unbiased results.
...
meta_RR %>% mutate(sd_lnRR = sqrt(VRR)) %>% select(lnRR, sd_lnRR, measure_change) %>% pivot_longer(cols = c(lnRR,sd_lnRR), names_to = "type", values_to = "value") %>% 
      ggplot(aes(x = 1,y = value)) + geom_violin() +geom_jitter(alpha = 0.3,aes(color = measure_change))+ facet_wrap(~type, scale = "free")
```
...
We counted the number of materials and study methods for each study. It can be found that almost every study studied only one zone (with only one exception) and one fish life stage.Therefore we assume that climate and life stage do not affect the variance composition of stuy. Instead, the internal variance of each study may be influenced by the method and the number of species it uses.
...
We performed the same statistics in terms of SPECIES. We found that each species was mostly studied at only one of its growth stages. Most species appeared in only one study, with little overlap of species between studies. And nearly half of the species in all experiments were studied under multiple study instruments (stimuli).
...
para_distribution <- function(data,paras){
   ...
   for (i in paras){
      c <- ensym(i)
      para_data <- data %>% group_by({{c}}) %>% summarise(mean_abs_lnRR = mean(abs_lnRR)) %>% 
          select(mean_abs_lnRR) %>% mutate(parameter = as.character(i))
      plot_data <- rbind(para_data,plot_data)
   }
   plot_data %>% ggplot(aes(sample = mean_abs_lnRR)) + stat_qq() + stat_qq_line() + facet_wrap(~parameter,scale = "free")
}
...
We therefore consider that the main experiment random effect belongs to the **study**, while the **stimulus type** as different experimental methods in a single **study** leads to different between-group variance, while other variance is contributed by differences in conditions between experiments(contain the use of **species**) and the experiment itself. From this we build a meta-analytic model.
...
4. There is no uniform direction for the experimental results  

Although our 95% confidence interval contains 0, the results obtained are still somewhat informative, with the average lnRR being 0.2161.

```{r}
predict(MLMA, transf = exp)
```

We convert lnRR back to Respose ratio (RR), and we expect a 95% chance of a true RR falling between 0.9-1.6.
We also built another meta-analysis model in which the random effects of species and study were independent.
...
```{r}
# orchaRd::orchard_plot(MLMR2, mod = "climate_fish_base", group = "study", data = meta_RR_climate,
...
```{r}
# orchaRd::orchard_plot(MLMR3, mod = "life_stage", group = "study", data = meta_RR_stage,
#     xlab = "log(Response ratio) (lnRR)", angle = 45)
# rm(meta_RR_stage)
```
...
```
```{r}
MLMR4 <- metafor::rma.mv(lnRR ~ climate_fish_base, V = VRR, method = "REML", 
            random = list(~climate_fish_base|species, ~cue_stimulus_type|study, ~1|residual),
            dfs = "contain", test = "t", data = meta_RR,struct = "HCS")
MLMR4
```
